package com.thaca.admin.controllers.internal;
...
public class InternalTenantController {
    ...
    @FwRequest(name = ServiceMethod.ADMIN_GET_TENANT, type = RequestType.INTERNAL)
    ...
    @FwRequest(name = ServiceMethod.ADMIN_GET_ALL_TENANTS, type = RequestType.INTERNAL)
    ...
    @FwRequest(name = ServiceMethod.ADMIN_GET_TENANTS_BY_IDS, type = RequestType.INTERNAL)
    public ResponseEntity<List<TenantInfoPrj>> getTenantsByIds(TenantDTO request) {
        return ResponseEntity.ok(process.process(request));
    }

    @PostMapping("/admin/tenants/get-full-by-ids")
    @FwRequest(name = ServiceMethod.ADMIN_GET_TENANTS_FULL_BY_IDS, type = RequestType.INTERNAL)

...
public interface TenantRepository extends JpaRepository<Tenant, Long>, JpaSpecificationExecutor<Tenant> {
    ...
    @Query(
        nativeQuery = true,
        value = """
        SELECT t.id, t.name, t.code, t.domain, t.status, t.plan_id as planId, t.expires_at as expiresAt, t.contact_email as contactEmail, t.logo_url as logoUrl, t.version, p.name as planName, p.code as planCode, p.type as planType FROM admin.tenants t
        LEFT JOIN admin.plans p ON t.plan_id = p.id
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM admin.tenants WHERE id = :id")
